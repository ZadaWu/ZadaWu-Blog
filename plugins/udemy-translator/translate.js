...
async function processSubtitle(subtitle) {
    ...
    try {
        ...
        translatedDiv.className = 'translation-span';
        ...
        // 移除旧的翻译（如果存在）
        const oldTranslation = subtitle.nextElementSibling;
        if (oldTranslation && oldTranslation.classList.contains('translation-span')) {
            oldTranslation.remove();
        }
        
        // 将翻译作为兄弟节点插入
        subtitle.parentNode.insertBefore(translatedDiv, subtitle.nextSibling);
        
        console.log('Translated:', currentText, '->', translatedText);
    } catch (error) {
        console.error('Translation error:', error);
    }
}
...
// 设置定时翻译
function setupTranslationTimer() {
    // 每5秒执行一次翻译
    setInterval(() => {
        ...
        subtitles.forEach(subtitle => {
            ...
            if (oldText === currentText) {
                console.log('Text unchanged, skipping:', currentText, oldText);
                return; // 跳过这个字幕
            }
    ...
    const subtitleContainerP = document.querySelector('.well--container--afdWD');
    if (subtitleContainerP) {
        subtitleContainerP.style.fontSize = '2.32rem';
    }
}
